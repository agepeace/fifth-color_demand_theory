...
本文在现有 Fifth-Color Demand Theory（FCD Theory）框架基础上，整理出一篇面向后续严格化证明的研究论文。文章不直接重证四色定理，而是围绕”第五色需求”这一局部-全局交叉对象，建立一套适用于平面图着色研究的结构语言。我们的核心目标是刻画如下命题：在最小围长大于等于 5 的任意平面图上，最多只有一个顶点具有持续第五色需求。本文给出该命题所需的定义系统、结构动机、传播图模型、势函数视角以及与不可避免构型、不可避免集之间的关系，并在第 6 节中建立一条从对象定义到逐类排除再到闭合回推的工作性证明链。该证明链在 3-正则平面图上已完成形式化骨架，并通过归约引理衔接一般平面图情形；当前仍需后续工作将局部刚性断言进一步细化为逐个微型局部构型的完全枚举证明。

This paper consolidates the current Fifth-Color Demand Theory framework into a research-oriented draft. Rather than re-proving the Four Color Theorem directly, it develops a structural language centered on fifth-color demand. The central target statement is the following: for any planar graph of girth at least five, at most one vertex can exhibit a persistent fifth-color demand. We provide the definitions, structural motivation, propagation model, potential-function viewpoint, and the relationship with unavoidable configurations and unavoidable sets. Section 6 establishes a working proof chain from object definitions through case-by-case elimination to a closure argument. This chain has been formalized as a proof skeleton on 3-regular planar graphs and connected to the general case via a reduction lemma; further work is needed to refine the local rigidity assertions into a complete enumeration proof of micro-level configurations.
...
本文的目标不是宣布上述核心命题已被完全证明，而是形成一篇逻辑自洽、术语完整、与现有仓库框架一致的研究论文，使后续工作能够直接在此基础上推进；就当前版本而言，正文已经形成一条工作性闭合证明链，但仍未达到最终定理级证明。
...
对于最小围长大于等于 5 的平面图，这一点尤其重要。由于不存在三角形，很多最紧局部闭锁会被削弱；由于每个顶点度数都恰为 3，需求传播的分支模式又被统一。于是，不可避免构型虽仍存在，但它们可支持的“多核需求共存机制”被显著压缩 [Borodin et al. 2005; Borodin and Glebov 2006]。
...
命题 6.1（中心命题，一般版）. 设 $G$ 为一个满足 $g(G)\ge 5$ 的平面图，$\varphi\in\mathcal{S}(G)$。在第 2.4 节与第 2.7 节固定的允许局部变换、可达状态邻域与持续性判别框架下，图 $G$ 中至多存在一个持续需求核。等价地说，不存在两个彼此独立的顶点能在同一研究框架下同时具有持续第五色需求。

Proposition 6.1 (central proposition, general version). Let $G$ be a planar graph with girth at least five, and let $\varphi\in\mathcal{S}(G)$. Under the present formalization of allowed local transformations, reachable-state neighborhoods, and persistence, at most one persistent demand core can occur in $G$. Equivalently, no two independent vertices can simultaneously exhibit persistent fifth-color demand in the same formal framework.

命题 6.1a（中心命题，3-正则版）. 设 $G$ 为一个满足 $g(G)\ge 5$ 的 3-正则平面图，$\varphi\in\mathcal{S}(G)$。在第 2.4 节与第 2.7 节固定的允许局部变换、可达状态邻域与持续性判别框架下，图 $G$ 中至多存在一个持续需求核。

Proposition 6.1a (central proposition, cubic version). Let $G$ be a cubic planar graph with girth at least five, and let $\varphi\in\mathcal{S}(G)$. Under the present formalization, at most one persistent demand core can occur in $G$.

**两版命题之间的关系**. 命题 6.1a 是命题 6.1 的特例（增加了 3-正则条件）。本文第 6.6 节至第 6.10 节的证明链直接证明的是命题 6.1a：引理 6.8–6.9.4 的论证显式使用了 3-正则条件（每个顶点恰好有三条边，使得"剩余自由度"的分析可穷尽枚举）。命题 6.1（一般版）则由如下归约引理衔接。

引理 6.1a（一般平面图到 3-正则图的归约）. 若命题 6.1a 成立，则命题 6.1 成立。

证明思路. 设 $G$ 为满足 $g(G)\ge 5$ 的平面图（不限于 3-正则——3-正则图是 $g(G)\ge 5$ 平面图的子类，此处 $G$ 可以是任意度数的），且 $G$ 中存在两个不同的持续需求核 $u,v$。对 $G$ 中每个度数 $\deg(w)\ge 4$ 的顶点 $w$，可以进行如下局部操作：在 $w$ 的邻域中添加一个细分顶点 $w'$，将 $w$ 的一条邻边 $(w,x)$ 替换为两条边 $(w,w')$ 与 $(w',x)$，使得 $\deg(w)$ 减 $1$ 而 $\deg(w')=2$。重复此操作直到所有顶点度数至多为 $3$，得到一个 3-正则图 $G'$（可能需要添加额外的悬挂边以处理奇数度顶点）。由于 $g(G)\ge 5$，细分操作不缩短围长，故 $g(G')\ge 5$。

关键在于：细分操作不改变 $u,v$ 的持续需求状态。这是因为 $F^{\dagger}(u,\varphi)=1$ 只依赖于 $u$ 的邻域颜色配置，而细分顶点 $w'$ 的着色可以在保持原着色 $\varphi$ 的限制下被唯一确定（$w'$ 只有两个邻点，其可用颜色集至少有 $2$ 种，故 $w'$ 不会产生新的需求核）。因此，$u,v$ 在 $G'$ 中仍是持续需求核，这与命题 6.1a 矛盾。

故命题 6.1a 蕴含命题 6.1。证毕。

说明. 上述归约引理将证明链的适用范围从 3-正则图扩展到一般平面图。归约的关键前提是细分操作不破坏持续需求核的存在性——这在 $F^{\dagger}$ 框架下成立，因为广义第五色需求只要求 $|D(v,\varphi)|\le 1$，而细分顶点的度数 $\le 2$，其可用颜色集 $|D|\ge 2$，不会成为新的需求核。后续引理 6.8–6.9.4 中对 3-正则条件的依赖，因此只在命题 6.1a 的证明中需要，而命题 6.1 由归约引理衔接。
...
第三步是平面排除。需要证明：在围长大于等于 5 的平面图中，这类双核连接结构无法存在。这里真正起作用的是平面嵌入、面长度下界与固定度数三者的共同约束，而不只是其中任何单一条件。
...
**条件 1**. 由 $g(G)\ge 5$ 知 $G$ 不含三角形与四边形，因此 $G$ 中每个面的边界长度至少为 $5$。$\Xi(u,v;\varphi)$ 的内部面是 $G$ 的面的子集，故其边界长度亦不小于 $5$。
...
$$
|P|\le 2\cdot g(G)\cdot (\Delta_{\partial}+1),
$$
...
该表只是把 IV 型内部的论证接口显式化，不新增新的数学内容，但能让读者一眼看出“先桥接，再分裂，再汇总”的结构顺序。
...
这些情形分别由断言 6.9.4a 与引理 6.9.4d-6.9.4h 处理。
...
推论 6.10.1（中心命题的工作性回收）. 在第 2.4 节、第 2.7 节与第 6.6 节至第 6.10 节的当前形式化框架下，命题 6.1a 得到工作性回收。进而由引理 6.1a（一般平面图到 3-正则图的归约），命题 6.1 也得到工作性回收。亦即：若 $G$ 为满足 $g(G)\ge 5$ 的平面图，且持续第五色需求按本文的允许局部变换与持续性判别来理解，则当前证明链支持 $G$ 中至多存在一个持续需求核。

证明. 命题 6.10 已经在 3-正则平面图上给出了”至多一个持续需求核”的工作性闭合，即命题 6.1a 的当前证明接口。由引理 6.1a，若命题 6.1a 在该接口下成立，则命题 6.1（一般版）也可在同一工作性框架下回收：因为任何满足 $g(G)\ge 5$ 的平面图中的双核共存，都可以通过细分操作归约为 3-正则图中的双核共存，而细分不破坏持续需求核的存在性。故命题 6.1 获得当前工作性证明链的支持。证毕。

这条推论的作用不是增加新的结构内容，而是把“第五步完成闭合”明确写成“中心命题已经在当前框架下获得工作性回收”。如此一来，第六节的末端逻辑就不再只是说明存在一条工作性闭合链，而是把这条闭合链显式落实为对命题 6.1 的证明接口。
...
本文当前聚焦于”最小围长大于等于 5 的平面图”，并非随意收缩研究范围，而是出于结构分析上的必要性。首先，平面性提供了强嵌入约束，使得需求传播链与需求核之间的相对位置不能任意交错。其次，围长大于等于 5 排除了三角形与四边形，从而移除了最短回路带来的极端局部闭锁效应，使得”需求持续存在”必须依赖更长的结构机制。在证明正文的具体引理中，3-正则作为一个额外的结构假设被使用——它统一了每个顶点的局部出度，使需求传播的分支复杂度保持恒定，更有利于形成可比较的局部类型。3-正则平面图是 $g(G)\ge 5$ 平面图的子类；中心命题的表述覆盖整个 $g(G)\ge 5$ 平面图类，而证明链先在 3-正则子类上建立（命题 6.1a），再通过引理 6.1a 的细分归约将结论向上覆盖到整个图类（命题 6.1）。这一归约方向与四色定理的经典归约一致：3-正则平面图的着色难度不低于一般平面图，因此子类上的结论可以向上传递。
...
本文完成了当前 FCD Theory 仓库的一次论文化整理。我们将“第五色需求”从一个局部着色失败事件，提升为可定义、可传播、可计量、可与经典构型理论对接的结构对象；补齐了不可避免构型、不可避免集、围长与度数等关键概念；并把核心研究目标明确表述为：在最小围长大于等于 5 的平面图上，最多只有一个顶点具有持续第五色需求。

但必须再次强调：本文当前给出的不是最终定理级证明，而是一条闭合的工作性证明链。它已经把中心命题推进到“对象语言清晰、候选引理分解明确、有限构型压缩与逐类排除形成闭合接口”的阶段，但仍需将局部刚性断言继续细化为逐个微型局部构型的完全枚举证明。

In conclusion, the paper turns the current FCD framework into a coherent structural research paper. It does not yet claim a final theorem-level proof, but it provides a closed working proof chain for the uniqueness of persistent fifth-color demand on planar graphs of girth at least five.
