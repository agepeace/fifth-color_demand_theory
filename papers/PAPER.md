...
1. S1：单口、无第二出口、无回返；对应 I 型端点模式。
2. D1：双口不可冗余配对、不可塌缩为单口；对应 II 型的双贴附端。
3. D2：双口同步配对、两端均需保留；对应 III 型的并列端。
4. R1：最短必要回返带、不可替代；对应 IV 型的短面带端。
5. R2：长回返带含扩张段、待由 6.9.4d-6.9.4h 排除；对应 IV 型的长面带端。

该表并不新增独立证明，而只是把命题 6.6.7 中的有限候选表 $\mathfrak{P}_{\partial}$ 具体化为可直接调用的五个工作性模式标签。
...
表 6.9.1-A（I 型单通道局部微型构型）. I 型排除实际只需检查以下三种局部情形：

1. I-1 回返边：路径内部顶点的第三边回到 $\Xi$ 或某端边界；
2. I-2 外部分支：第三边连向可承担传播的外部分支；
3. I-3 外部无贡献邻点：第三边连向不参与任何需求见证的外部顶点。

这三类正对应下文关于外部邻点 $w$ 的三分讨论。
...
表 6.9.2-A（II 型汇总区局部微型构型）. II 型排除实际只需检查以下三种局部情形：

1. II-1 双口提前合流：双贴附端在到达单口前已经塌缩；
2. II-2 并行支路：连接区产生独立于主通道的分担支路；
3. II-3 一路压力失效：某一路经过无贡献邻点而退化为冗余。

这三类分别对应双口不可冗余性、无并行通道约束与单口锁定要求之间的冲突。
...
表 6.9.3-A（III 型并列同步局部微型构型）. III 型排除实际只需检查以下四种局部情形：

1. III-1 对应关系交换：两端双口匹配在中途交换或错位；
2. III-2 无代价合流：两组传播在内部合流而不产生额外代价；
3. III-3 完全分离成并行带：两组传播始终分离，强迫实质并行传播带；
4. III-4 外部无贡献邻点退化：第三边连向无贡献邻点，使某组贴附压力失效。

这四类把“并列同步”从整体直觉拆成可逐项检查的局部失败模式。
...
表 6.9.4-A（IV 型回返面带局部微型构型）. IV 型排除实际只需检查以下五种局部情形：
...
在断言 6.9.4a 基础上，再看 IV 型的主矛盾。IV 型之所以是最后残余候选，正在于它试图利用“一次最短回返”同时承担两项任务：一方面为路径 $P$ 提供局部缓冲，使来自一端的阻塞压力不会立刻在另一端被局部解锁；另一方面又保持整个连接体的最短性与极小性，不至于退化成更大的冗余见证结构。但这两个要求在围长大于等于 $5$ 的 3-正则平面图中彼此并不相容。
...
证明. 取 $R$ 的一段扩张边界 $S$。由断言 6.9.4a，$S$ 上每一段都必须对双端锁定有实质贡献，且不能被更短弦替代，也不能在有限冻结球内被局部剪除。现在考察 $S$ 在平面 3-正则嵌入中的可能角色。
