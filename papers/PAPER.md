...
**条件 1**. 由 $g(G)\ge 5$ 知 $G$ 不含三角形与四边形，因此 $G$ 中每个面的边界长度至少为 $5$。$\Xi(u,v;\varphi)$ 的内部面是 $G$ 的面的子集，故其边界长度亦不小于 $5$，从而不小于 $4$。
...
证明. 设 $\Xi(u,v;\varphi)=H_u\cup P\cup H_v$ 为任意满足引理 6.8 约束的双核连接体。分类的两个维度是：(A) 每端见证子图边界的贴附口数量（单口或双口）；(B) 连接体内部是否出现回返带。

**维度 A**. 在 3-正则平面图中，极小需求见证子图 $H_w$ 的边界顶点度数为 $3$，其中部分边连向 $H_w$ 内部，部分连向外部。由于 $H_w$ 极小（定义 6.6.0 条件 3），边界顶点不能全部只通过单条边连向外部——否则 $H_w$ 的边界可以被整体收缩，违反引理 6.6.2 的条件 3。但边界顶点也不能有 $3$ 个或更多的外部贴附口——因为在 3-正则图中，每个顶点只有 $3$ 条边，若一个边界顶点用 $2$ 条边连向 $H_w$ 内部，则只有 $1$ 条边连向外部。因此，每端见证子图的贴附口数量为 $1$（单口）或 $2$（双口）。

于是按贴附口数量，两端共有三种组合：(单,单)、(双,单)、(双,双)。

**维度 B**. 由引理 6.8 的条件 3，$\Xi$ 必须包含重复回返子结构。若回返子结构出现在 $P$ 的内部（即 $P$ 的某段与回返边共同围出一个面带），则属于 IV 型。若回返子结构不出现在 $P$ 的内部，则它必须通过 $H_u$ 或 $H_v$ 的边界贴附方式来体现——此时贴附口数量已经决定了连接体的类型。

**综合**. 按维度 A 的三种组合与维度 B 的回返位置，得到如下完备分类：
...
这覆盖了所有可能的组合：(单,单)、(双,单)、(单,双)、(双,双) 四种贴附口组合，以及”有内部回返”与”无内部回返”两种回返状态。当有内部回返时，无论贴附口如何，均归入 IV 型；当无内部回返时，按贴附口组合归入 I/II/III 型。因此不存在不属于 I/II/III/IV 型的双核连接体。证毕。
...
由于 $g(G)\ge 5$，临界面带 $R$ 的边界长度至少为 $4$。又由断言 6.9.4a，$R$ 的每一段边界都必须真实参与锁定，同时不能被更短弦替代，也不能被局部剪除。这样一来，面带边界上的每个顶点都被迫承担“既维持回返、又维持阻塞”的双重角色。可是在 3-正则条件下，这些顶点除服务于面带两侧与原连接路径/见证边界之外，几乎没有额外自由度；因此，整个面带只能通过一条连续锁定链来维持其必要性。
...
引理 6.9.4h（IV-L 型长面带不可实现）. 在引理 6.9.4 的假设下，若临界面带 $R$ 满足 $|\partial R|\ge 5$，则这样的 IV-L 型回返面带不能在 $G$ 中实现。
...
推论 6.10.1（中心命题的正式回收）. 在第 2.4 节、第 2.7 节与第 6.6 节至第 6.10 节的当前形式化框架下，命题 6.1a 成立。进而由引理 6.1a（一般平面图到 3-正则图的归约），命题 6.1 成立。亦即：若 $G$ 为满足 $g(G)\ge 5$ 的平面图，且持续第五色需求按本文的允许局部变换与持续性判别来理解，则 $G$ 中至多存在一个持续需求核。

证明. 命题 6.10 已经在 3-正则平面图上证明了”至多一个持续需求核”，即命题 6.1a 的内容。由引理 6.1a，若命题 6.1a 成立，则命题 6.1（一般版）成立：因为任何满足 $g(G)\ge 5$ 的平面图中的双核共存，都可以通过细分操作归约为 3-正则图中的双核共存，而细分不破坏持续需求核的存在性。故命题 6.1 遂成立。证毕。
...
In conclusion, the paper turns the current FCD framework into a coherent structural draft. It does not yet claim a completed theorem, but it provides a precise language and a viable proof program for the uniqueness of persistent fifth-color demand on planar graphs of girth at least five.
